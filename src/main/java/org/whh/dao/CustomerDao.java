...
public interface CustomerDao extends PagingAndSortingRepository<Customer, Long>,JpaSpecificationExecutor<Customer>
{
	...
	Page<Customer> findUnRecommendCustomer(boolean isRecommend,
			int consumeLevel, Pageable pageable);

	@Query("from Customer c where c.isRecommend = ?1 and c.province like ?2 and c.consumeLevel = ?3")
	Page<Customer> findUnRecommendCustomerWithProvince(boolean isRecommend,
			String province, int consumeLevel, Pageable pageable);
